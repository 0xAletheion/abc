...
const generatedPath = path.join(directory, '.rakuten-multi-watch-runtime-v3.mjs');
    ...
    try {
      const found = await frame.evaluate(({ value, kind, marker }) => {
        ...
        const addCandidate = element => {
          ...
          const idealAreaPenalty = Math.abs(area - 13_000);
          const score =
...
async function clickByGeometry(page, watch, value, kind, watchResult) {
  ...
  for (const offset of yOffsets) {
    ...
    if (await waitForVariantSelection(page, value, kind, 2_400)) return true;
  }
...
source = replaceExactlyOnce(source, selectionCallPattern, selectionCallReplacement, 'selectVariants call site');
